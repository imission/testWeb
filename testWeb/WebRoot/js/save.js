...
function buildItemValue(itemDivId) {
	...
	$(subitemObjs).each(
		...
		function(index, ele) {
			...
			var check_id, item_num, sub_num, stype, value_type, value_str, value_multi;

			check_id = $(subitemObj).children("[sn=check_id]")
					.val();
			item_num = $(subitemObj).children("[sn=item_num]")
					.val();
			sub_num = $(subitemObj).children("[sn=sub_num]").val();
			stype = $(subitemObj).children("[sn=stype]").val();
			value_type = $(subitemObj).children("[sn=value_type]")
					.val();
			value_str = $(subitemObj).children("[sn=value_str]")
					.val();
			...
			// --- 判断值 begin ---
			if (check_id == null
					|| typeof (check_id) == 'undefined')
				check_id = 0;
			if (item_num == null
					|| typeof (item_num) == 'undefined')
				item_num = "";
			...
			if (stype == null || typeof (stype) == 'undefined')
				stype = "";
			if (value_type == null
					|| typeof (value_type) == 'undefined')
				value_type = "";
			if (value_str == null
					|| typeof (value_str) == 'undefined')
				value_str = "";
			if (value_multi == null
					|| typeof (value_multi) == 'undefined')
				value_multi = "";
			// --- 判断值 end ---
	...
	itemJson += ("'item_num':'" + item_num + "'" + ",'time':'" + time + "'"
